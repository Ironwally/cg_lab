...
// Aufgabe_3 - Standard constructor
template <class FLOAT_TYPE, size_t N>
Vector<FLOAT_TYPE, N>::Vector() {
  // Do nothing from aufgabe does not work (vector filled with random values test with zeros
...
FLOAT_TYPE Vector<FLOAT_TYPE,N>::length() const {
  ...
  return sqrt(sum);
  //return sqrt(vector.square_of_length());
}
