...
template <int N>
static std::array<SDL_Point, N> convertSurfaceToPoints(SDL_Surface* surface) {
  std::array<SDL_Point, N> points;
  int index = 0;
...
void SDL2Renderer::
renderSpaceship(Vector2df position, float angle) {
/*
  SDL_Surface* texture = IMG_Load("../assets/images/spaceship.drawio.svg");
  //translate SDL_Surface* to std::array<SDL_Point, n>
  if (!texture) {
    std::cerr << "Failed to load texture: " << IMG_GetError() << std::endl;
    return;
  }

  static auto ship_points = convertSurfaceToPoints<126000>(texture);

  SDL_Surface *spaceship = IMG_Load("../assets/images/spaceship.drawio.svg");
  SDL_Texture *spaceship_texture = SDL_CreateTextureFromSurface(renderer, spaceship);
  SDL_FreeSurface(spaceship);
  SDL_RenderCopy(renderer, spaceship_texture, NULL, &texture_destination3);
  */
  static std::array<SDL_Point, 18> ship_points{SDL_Point{-6, 3},
                                              SDL_Point{-6,-3},
                                              SDL_Point{-10,-6},
                                              SDL_Point{ 14, 0},
                                              SDL_Point{-10, 6},
  ...
  //SDL_SetRenderDrawColor( renderer, 0xF5, 0xF5, 0x06, 0x5F );
}

void SDL2Renderer::render(Spaceship * ship) {
  static SDL_Point flame_points[] { {-24, 12}, {-48, 0}, {-24, -12} };
