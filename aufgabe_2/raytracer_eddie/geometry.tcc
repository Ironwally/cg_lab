...
// Aufgabe 2 Anfang
// Prüfung, ob Kugeln sich überschneiden
template <class FLOAT, size_t N>
bool Sphere<FLOAT, N>::intersects(Sphere<FLOAT, N> sphere) const {
  // Abstand der Mittelpunkte der beiden Kugeln berechnen
  FLOAT distanceSquared = (center - sphere.center).square_of_length();

  // Summe der Radien mit dem Abstand vergleichen
  FLOAT radiusSum = radius + sphere.radius;
  return distanceSquared <= radiusSum * radiusSum;
}

// Prüfung, ob Punkt in Kugel oder auf Oberfläche
template <class FLOAT, size_t N>
bool Sphere<FLOAT, N>::inside(const Vector<FLOAT, N> p) const {
  // Abstand des Punktes zum Mittelpunkt der Kugel berechnen
  FLOAT distanceSquared = (p - center).square_of_length();

  // Prüfen, ob Abstand kleiner/gleich dem Radius ist
